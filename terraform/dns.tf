# Cloudflare DNS managed inside Terraform so the ACM validation record exists
# BEFORE aws_acm_certificate_validation waits on it. (v1's post-apply dns job
# deadlocked on first run: validation waited for a record that was only
# created after apply finished.)
